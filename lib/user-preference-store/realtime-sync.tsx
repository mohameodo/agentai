  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(userRef, (doc) => {
      if (doc.exists()) {
        ...
        // Only refresh user data if there are actual changes that aren't just preference updates
        const hasNonPreferenceChanges = Object.keys(userData).some(key => 
          key !== 'preferences' && 
          key !== 'preferred_model' && 
          key !== 'updated_at' &&
          userData[key] !== (user as any)?.[key]
        )
        
        if (hasNonPreferenceChanges) {
          refreshUser()
        }
      }
    }, (error) => {
